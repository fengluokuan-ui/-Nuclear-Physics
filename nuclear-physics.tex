...
\begin{abstract}
无中微子双贝塔衰变（$0\nu\beta\beta$）是同时连接原子核结构、中微子质量起源和超出标准模型物理的重要稀有过程。本文从双贝塔衰变的能量条件、轻子数计数和实验能谱特征出发，比较标准模型允许的 $2\nu\beta\beta$ 与轻子数破坏的 $0\nu\beta\beta$；随后讨论 Majorana 中微子假设、有效 Majorana 质量 $m_{\beta\beta}$、质量排序和 Majorana 相位对实验解释的影响。在半衰期公式中，核矩阵元是连接实验寿命极限与粒子物理参数的关键理论输入，其不确定性直接限制 $m_{\beta\beta}$ 和非标准机制耦合的提取精度。围绕这一问题，本文进一步概述实验灵敏度、格点 QCD 对少体长程矩阵元的约束、手征有效场论到 N$^2$LO 的系统算符补全，以及多参考协变密度泛函理论对不同轻子数破坏机制的核矩阵元计算。短程项、ultrasoft 项、loop 项以及多机制干涉对未来高灵敏度实验结果的解释均不可忽略。
\end{abstract}

\noindent\textbf{关键词：}无中微子双贝塔衰变；Majorana 中微子；有效 Majorana 质量；核矩阵元；格点 QCD；手征有效场论；多机制轻子数破坏
...
中微子振荡实验已经确认中微子具有非零质量，这一事实本身已经超出了标准模型的原始框架。然而，振荡实验只能给出质量平方差和混合角，不能判断中微子质量的绝对标度，也不能回答中微子是否等同于自身的反粒子。若中微子是 Majorana 粒子，则理论上允许破坏总轻子数两个单位的过程。无中微子双贝塔衰变正是这类过程在低能核体系中的典型表现：
\[
	(A,Z)\rightarrow(A,Z+2)+2e^- .
\]
它一旦被观测到，将直接表明自然界存在轻子数破坏，并为 Majorana 中微子质量和高能新物理提供重要线索。

从原子核物理角度看，双贝塔衰变之所以成为理想探针，是因为某些偶偶核的单贝塔路径受能量或核结构选择规则抑制，而双贝塔路径仍然能量允许。标准的双中微子双贝塔衰变（$2\nu\beta\beta$）已经在多个核素中被观测到，它是标准模型允许的二阶弱过程；无中微子模式（$0\nu\beta\beta$）则在末态缺少两个反中微子，因而具有 $|\Delta L|=2$。实验上，二者的核心区别体现在电子总能量谱：$2\nu\beta\beta$ 给出连续谱，$0\nu\beta\beta$ 则在 $Q_{\beta\beta}$ 附近形成单能峰。这一清晰的谱学特征使得深地下、低本底、高能量分辨率实验可以对极长半衰期进行搜索。

如果 $0\nu\beta\beta$ 由轻 Majorana 中微子交换主导，其半衰期可与有效 Majorana 质量 $m_{\beta\beta}$ 联系起来。$m_{\beta\beta}$ 由中微子质量本征值、PMNS 混合矩阵元素和未知 Majorana 相位共同决定，因此不同质量排序下的允许区域具有明显差异。不过，实验测得或限制的是半衰期，而不是直接的中微子质量。将半衰期转化为 $m_{\beta\beta}$ 或其他轻子数破坏耦合，必须依赖相空间因子和核矩阵元。后者涉及强相互作用、核多体关联、弱流算符和有效低能常数，是当前理论解释中的主要不确定性来源。

围绕核矩阵元问题，近年来的研究正在从单一核模型估算转向多尺度、系统误差可控的框架。一方面，格点 QCD 可从强相互作用第一性原理出发，为少体长程矩阵元和低能常数提供输入；另一方面，手征有效场论能够按动量阶数组织 $0\nu\beta\beta$ 算符，把长程中微子交换、短程接触项、ultrasoft 中微子贡献和 loop 修正统一纳入同一展开。与此同时，多参考协变密度泛函理论等核多体方法可在真实候选核中比较轻中微子交换、重中微子交换和右手流等不同机制的核矩阵元。本文按照“基本物理--粒子物理含义--核矩阵元--实验现状--理论进展”的逻辑展开，重点说明 $0\nu\beta\beta$ 如何把中微子性质、核结构计算和未来实验发现联系起来。
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.72\textwidth]{doubledetadecay.jpg}
	\caption{$2\nu\beta\beta$ 连续谱与 $0\nu\beta\beta$ 在 $Q_{\beta\beta}$ 处窄峰的示意图。图取自教材\cite{Lu2000NuclearPhysics}。}
	\label{fig:spectrum}
\end{figure}
...
\begin{figure}[htbp]
	...
	\caption{$m_{\beta\beta}$ 随最轻中微子质量变化的允许区域。红色和绿色带分别对应正常排序与倒置排序，灰色区域表示实验限制（图片取自文献\cite{GomezCadenas2023}）。}
	\label{fig:mbb}
\end{figure}
...
无中微子双贝塔衰变的核心物理意义在于，它把一个可在原子核中寻找的稀有衰变过程，与轻子数破坏、中微子 Majorana 性质以及超出标准模型的新物理直接相连。与标准模型允许的 $2\nu\beta\beta$ 不同，$0\nu\beta\beta$ 的末态没有反中微子，轻子数改变两个单位；实验上则表现为两个电子总能量在 $Q_{\beta\beta}$ 附近的窄峰。因此，若未来实验观测到可信信号，将构成轻子数破坏的直接证据，并强烈支持中微子具有 Majorana 质量成分。

在轻 Majorana 中微子交换图像下，$0\nu\beta\beta$ 半衰期与有效 Majorana 质量 $m_{\beta\beta}$ 相关。该量不仅依赖中微子质量排序和绝对质量标度，还受未知 Majorana 相位控制，因此可能出现相干增强或抵消。与此同时，半衰期公式中的核矩阵元决定了实验寿命极限如何转化为粒子物理参数限制。不同核模型给出的矩阵元仍存在差异，$g_A$ 淬灭、短程关联、闭合近似、高阶弱流以及低能常数等问题都会影响最终解释。

近期理论进展正在逐步缩小这些不确定性。格点 QCD 为从强相互作用出发确定少体输入和长程矩阵元提供了途径；手征 EFT 将 $0\nu\beta\beta$ 算符按阶数系统组织，并在 N$^2$LO 纳入 ultrasoft 与 loop 等修正，显示高阶项对矩阵元可产生不可忽略的影响；MR-CDFT 等多体方法则有助于在真实重核中比较不同轻子数破坏机制。未来实验若进入 $10^{27}$--$10^{28}$ 年半衰期灵敏度区间，理论上必须同时给出更可靠的核矩阵元、机制判别方法和跨核素一致性检验。因此，$0\nu\beta\beta$ 研究不仅要判断是否存在信号，还要在发现或排除信号后，对中微子质量机制和新物理来源作出定量解释。
